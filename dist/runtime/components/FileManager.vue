...
let fileManager = class extends Vue {
  constructor() {
    super(...arguments);
    ...
    this.input_rules = [
      (value) => value.indexOf(" ") === -1 || "Name contains spaces!"
    ];
  }
  get headers() {
    const headers = [
  ...
  doUploadFile(file) {
    ...
    return new Promise((resolve) => {
      ...
      this.$axios.post(
        this.params.apiUrl + "/server/files/upload",
        formData,
        {
          ...
          onUploadProgress: (progressEvent) => {
            ...
            if (this.uploadSnackbar.lastProgress.time) {
              ...
              if (time)
                this.uploadSnackbar.speed = data / (time / 1e3);
            }
        ...
        const filename2 = result.data.item.path.substr(result.data.item.path.indexOf("/") + 1);
  ...
  refreshMetadata(data) {
    ...
    for (let i = data.pageStart; i < data.pageStop; i++) {
      if (items[i] && !items[i].isDirectory && !items[i].metadataPulled) {
        ...
        this.$emit("serverFilesMetadata", { filename }, { action: "files/getMetadata" });
      }
    }
  }
  clickRowGoBack() {
  ...
  getSmallThumbnail(item) {
    if (item.thumbnails?.length) {
      ...
      if (thumbnail && "relative_path" in thumbnail)
        return "http://" + this.params.apiUrl + "/server/files/" + this.currentPath + "/" + thumbnail.relative_path + "?timestamp=" + item.modified;
    }
    return "";
  }
  getBigThumbnail(item) {
...
], fileManager.prototype, "closeable", 2);
__decorateClass([
  Prop({ type: Boolean, default: false })
], fileManager.prototype, "noPrint", 2);
__decorateClass([
  Prop({ type: Array, default: () => [] })
], fileManager.prototype, "loadings", 2);
__decorateClass([
  Prop({ type: Object, default: { free: 0, total: 0, used: 0 } })
], fileManager.prototype, "diskUsage", 2);
__decorateClass([
  Prop({ type: Array, default: () => [] })
], fileManager.prototype, "validGcodeExtensions", 2);
__decorateClass([
  Prop({
    type: Object,
    default: () => {
...
], fileManager.prototype, "params", 2);
__decorateClass([
  Prop({
    type: Object,
    default: () => {
...
], fileManager.prototype, "printerInfo", 2);
__decorateClass([
  Prop({ type: String, default: "" })
], fileManager.prototype, "currentPath", 2);
__decorateClass([
  PropSync("options", {
    type: Object,
    default: () => {
...
], fileManager.prototype, "optionsSync", 2);
__decorateClass([
  PropSync("filesForCopyDialog", { type: Array, default: () => [] })
], fileManager.prototype, "filesForCopyDialogSync", 2);
__decorateClass([
  Model("", { type: Array, default: () => [] })
], fileManager.prototype, "filetree", 2);
__decorateClass([
  Watch("filetree", { deep: true })
], fileManager.prototype, "filetreeChanged", 1);
__decorateClass([
  Watch("currentPath")
], fileManager.prototype, "currentPathChanged", 1);
